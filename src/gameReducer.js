...
export const gameReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'FLIP_CARD': {
      ...
      // Check for bust
      if (scores.yuck >= 3) {
        // Player busted - check if opponent finished
        if (state.opponentFinished) {
          // Both finished, go to round_end
          return {
            ...state,
            ...
            nextCardPreview: null,
            phase: 'round_end',
            roundResult: state.opponentFinalScore > 0 ? 'loss' : 'tie',
            currentTurn: 'done',
            message: `Busted! Match over!`
          };
        }
        ...
        return {
          ...
          message: `Busted! Yuck: ${scores.yuck}. Waiting for opponent...`
        };
      }
        ...
        if (newScores.yuck >= 3) {
          ...
          if (state.opponentFinished) {
            return {
              ...
              roundResult: state.opponentFinalScore > 0 ? 'loss' : 'tie',
              currentTurn: 'done',
              message: `Sticky Onions busted you! Match over!`
            };
          }
          ...
          return {
            ...
            nextCardPreview: null,
            currentTurn: 'opponent',
            message: `Sticky Onions busted you! Waiting for opponent...`
          };
        }
      }
    ...
    case 'PLAY_BREAD': {
      ...
      // If opponent already finished, calculate result immediately
      if (state.opponentFinished) {
      ...
      return {
        ...
        nextCardPreview: null,
        currentTurn: 'opponent',
        message: `You finished: ${scores.flavor} flavor!${coldestCut ? ' COLDEST CUT!' : ''} Opponent's turn...`
      };
    }
    ...
    case 'CLAIM_REWARD': {
      if (state.roundResult === 'loss') {
        ...
        localStorage.setItem('sammich_losses', (state.losses + 1).toString());
